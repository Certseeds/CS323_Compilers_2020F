...
InterCode *translate_Exp_Assign_Exp(Node *exp, const std::string &place) {
    auto *const leftNode = exp->get_nodes(0);
    auto stringOfLeft = std::get<string>(leftNode->get_nodes(0)->value);
    auto *const rightNode = exp->get_nodes(2);
...
InterCode *translate_functionInvoke(Node *stmt) {
    Node *subnodes[3]{stmt->get_nodes(0), stmt->get_nodes(1), stmt->get_nodes(2)};
    return nullptr;
}
...
InterCode *translate_functionWithParamInvoke(Node *stmt) {
    ...
    if (functionName == "write"){
        stmt->interCode = new InterCode(InterCodeType::WRITE);
        stmt->interCode->SingleElement = new Operand(OperandType::VARIABLE,paramName);
        stmt->interCode->print();
    }
    return nullptr;
}
